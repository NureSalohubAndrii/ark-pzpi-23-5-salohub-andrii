...
const char* SERVER_BASE_URL = "http://192.168.1.5:3000/api/iot";
...
void loadSettings() {
  ...
  Serial.printf("Active Interval: %lu ms (%.1f sec)\n", 
                config.activeIntervalMs, config.activeIntervalMs / 1000.0);
  Serial.printf("Idle Interval: %lu ms (%.1f min)\n", 
                config.idleIntervalMs, config.idleIntervalMs / 60000.0);
...
void syncWithServer() {
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == 200) {
      ...
      Serial.println("CRITICAL: Car with VIN " + config.vin + " not found in database!");
...
void simulateSensors() {
  ...
  float rawVoltage = currentData.engineRunning 
    ? (14.2 + random(-5, 5) / 10.0)   
    : (12.5 - random(0, 5) / 100.0);  
    
  float rawHumidity = 50.0 + random(-100, 100) / 10.0;

  currentData.fuelLevel = smoothData(
    currentData.fuelLevel, 
    rawFuel, 
    config.smoothingAlphaFuel  
  );
  if (currentData.fuelLevel < 0) currentData.fuelLevel = 0;

  currentData.batteryVoltage = smoothData(
    currentData.batteryVoltage, 
    rawVoltage, 
    config.smoothingAlphaBattery  
  );
...
void loop() {
  ...
  unsigned long interval = currentData.engineRunning 
    ? config.activeIntervalMs 
    : config.idleIntervalMs;
  
  if (currentTime - lastTelemetrySend >= interval) {
    Serial.println("\nPeriodic telemetry send");
    Serial.printf("Mileage: %d km | Fuel: %.1f%% | Battery: %.2fV | Humidity: %.1f%%\n",
                  currentData.mileage, currentData.fuelLevel, 
                  currentData.batteryVoltage, currentData.humidity);
